...
# 配置初始化
def init_ship_cfg(_cfg_path):
    _ship = input_args(_cfg_path, 'ship')
    global cfg_path
    cfg_path = _cfg_path
    ship.update(_ship)


def init_ship_trips(_xml_path, _vehicle_names):
    # 模拟场景名称
    simulator_name = ship['simulator']['name']
